...
COPY Fmask_4_6_Linux_mcr.install .
RUN chmod +x Fmask_4_6_Linux_mcr.install && \
    ./Fmask_4_6_Linux_mcr.install -mode silent -agreeToLicense yes && \
    rm Fmask_4_6_Linux_mcr.install

COPY run_fmask.sh /usr/local/bin/run_fmask.sh
RUN chmod +x /usr/local/bin/run_fmask.sh

COPY run_Fmask_4_6.sh /usr/GERS/Fmask_4_6/application/run_Fmask_4_6.sh
RUN chmod +x /usr/GERS/Fmask_4_6/application/run_Fmask_4_6.sh
